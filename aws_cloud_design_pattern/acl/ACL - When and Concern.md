---
title: 什麼時候該使用 ACL（防腐層）模式？有哪些顧慮？
tags: [reading-note, architecture, ddd, patterns]
status: draft
source: aws-cloud-design-patterns.pdf
---

# 什麼時候該使用 ACL（防腐層）模式？有哪些顧慮？

## 摘要

- 在遷移或外部整合時，用 ACL 隔離並轉換不相容的模型/語義。[[#^ref-when-migrate-model-mismatch|1]]
- 主要風險：維運成本、單點故障、延遲、擴展瓶頸，以及「暫時層」演變成技術債。[[#^ref-concern-ops-overhead|5]]
- 緩解方式：韌性設計、可觀測性、效能測試、可擴展設計，以及規劃退場（拆除）。[[#^ref-mitigate-retry-cb|10]]

## 何時使用

- 單體遷移到微服務後，新舊領域模型/語義不一致，需要透過 ACL 做翻譯以維持穩定通訊。[[#^ref-when-migrate-model-mismatch|1]]
- 兩個系統必須交換資料，但語義/模型不相容，且修改任一方成本過高或不可行。[[#^ref-when-semantics-incompatible|2]]
- 與不受你控制的外部/第三方系統整合時，用 ACL 隔離外部變動與語義差異。[[#^ref-when-external-system|3]]
- 想把遷移影響降到最低：讓呼叫端維持原介面，由 ACL 在背後做路由/轉換以降低中斷與風險。[[#^ref-when-minimize-caller-change|4]]

## 何時不建議使用

- 對小型系統（重構複雜度低）而言，直接以微服務架構重寫，可能比透過 ACL 做遷移更有效率。[[#^ref-when-not-small-apps|15]]
- 評估 ACL 是過渡方案還是長期方案；若會變成長期負擔，就要重新審視是否值得。[[#^ref-when-not-long-term-solution|16]]
...
- 技術債/退場：若 ACL 只是過渡方案，必須被追蹤並在呼叫端完成遷移後拆除。[[#^ref-concern-techdebt-decommission|9]]
...
- 把 ACL 納入既有 CI/CD、監控與維運流程，避免成為孤島元件。[[#^ref-mitigate-integrate-cicd-obs|14]]
